...
module.exports = (() => {
   ...
   * 
   * @returns {string[]|Promise<string[]>} An array of every filepath inside the root directory (opts.filePath).
   */
  ...
  function recursiveReaddirInternal(opts) {
    ...
    try {
      ...
      subFiles = [
        ...folders.map(x => {
          let result = recursiveReaddirInternal({
            ...opts,
  ...
  async function recursiveReaddirInternalAsync(opts) {
    ...
    try {
      ...
      subFiles = [
        ...(await Promise.all(
          folders.map(async x => {
            let result = await recursiveReaddirInternal({
              ...opts,
              filePath: opts.filePath + '/' + x.name,
              excludeDirs: opts.excludeDirs
                .filter(x => x.startsWith(x))
